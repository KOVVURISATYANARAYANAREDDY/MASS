...
class EncDecTrainer(Trainer):
    ...
    def restricted_mask_sent(self, x, l, span_len=100000):
        ...
        mask_len = round(int(min(l)) * self.params.word_mass)
        ...
        for i in range(l.size(0)):
            #mask_len = round(len(x[:l[i], i]) * self.params.word_mass)

            unmasked_tokens = [0 for j in range(l[i] - mask_len - 1)]
            segs = self.get_segments(mask_len, span_len)
            ...
            try:
                ...
                print("mask_len, unmasked_tokens, segs, words, shuf_segs, pos_i", mask_len, unmasked_tokens, segs, words, shuf_segs, pos_i)
                print("x: ", x)
                output_i = words[pos_i].copy()
    ...
    def mass_step(self, lang, lambda_coeff):
        ...
        #print("X_, len_", x_, len_)
        #print(int(params.lambda_span))
