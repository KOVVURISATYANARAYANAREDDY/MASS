...
if [ "$SRC" == "ro" ]; then
  ...
  SRC_PREPROCESSING="bash $REPLACE_UNICODE_PUNCT | bash $NORM_PUNC -l $SRC |  bash $REM_NON_PRINT_CHAR |                                           bash $TOKENIZER -l $SRC -no-escape -threads $N_THREADS"
fi
if [ "$TGT" == "ro" ]; then
  ...
  TGT_PREPROCESSING="$bash REPLACE_UNICODE_PUNCT | $NORM_PUNC -l $TGT | $REM_NON_PRINT_CHAR |                                            $TOKENIZER -l $TGT -no-escape -threads $N_THREADS"
fi

# tokenize data
if  [[ -f "$SRC_TOK" ]]; then
  echo "Tokenize $SRC monolingual data..."
  eval "bash SRC_RAW | $SRC_PREPROCESSING > $SRC_TOK"
fi
